object Functions {
    ...
    def listOfAllCharacters(list:List[String]):List[Char] = list.toList.flatten


    // return the first string in a list, or if list is empty, return "nada"
    def firstElementInListOrNada(list:List[String]) = 
    
    if  (list.isEmpty) "nada" else list.head
    
    // find the longest string in given list of strings, or "nada" if empty
    // and reverse the result before returning it
    def longestNameInList(names:List[String]) = if (names.isEmpty) "nada".reverse else names.reduceLeft((x,y) => if (x.length > y.length) x else y).reverse
